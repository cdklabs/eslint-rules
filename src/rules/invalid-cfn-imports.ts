...
export function create(context: Rule.RuleContext): Rule.NodeListener {
  ...
  return {
    ImportDeclaration: node => {
      ...
      // ImportSpecifiers can also contain literals.
      type ImportSpecifierWIdentifier = Omit<ImportSpecifier, 'imported'> & { imported: Identifier };

      const cfnImports: ImportSpecifierWIdentifier[] = [];
      const otherImports: ImportSpecifierWIdentifier[] = [];
      node.specifiers.forEach(e => {
        if (e.type === 'ImportSpecifier' && isIdentifier(e.imported)) {
          if (e.imported.name.startsWith('Cfn')) {
            cfnImports.push(e as any);
          } else {
            otherImports.push(e as any);
          }
        }
      });
...
function checkIfImportedLocationIsAnAlphaPackage(location: string): boolean {
